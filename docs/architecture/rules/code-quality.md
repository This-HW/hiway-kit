...
> 이 문서는 이 킷에서 적용하는 코드 품질 기준과 그 이유를 설명합니다.

---

## 1. 함수 크기 — 왜 수치를 두지 않는가

주입 룰(`plugins/common/rules/code-quality.md`)은 줄 수·파라미터 수·중첩 깊이의 **고정 상한을
두지 않는다.** 길이·파라미터·중첩이 **커지는 것 자체가 쪼갤 신호**라고만 말한다.

- 고정 수치는 언어·도메인마다 맞는 값이 다르다. 같은 20줄이 어떤 언어에서는 길고 어떤
  언어에서는 짧다.
- 수치는 **지켜야 할 목표**로 읽히기 쉽다 — 한 가지 일을 하는 25줄 함수를 억지로 쪼개거나,
  여러 일을 하는 18줄 함수를 통과시킨다. 판단 기준은 줄 수가 아니라 **한 가지 일을 하는가**다.
- 소비자 프로젝트에 이미 린터(복잡도·길이 규칙)가 있으면 그 설정이 수치의 정본이다.

파라미터가 늘어날 때 흔히 쓰는 처방은 아래와 같다.
...
처리할 수 있는 타입은 처리하고, 나머지는 컨텍스트를 보존해 상위로 던집니다. 의도적으로
fail-open(실패를 삼키고 진행)하는 자리라면 **왜 그런지 주석으로 남깁니다** — 룰이 그렇게 요구합니다.
...
- [ ] 각 함수가 한 가지 일만 하는가?
- [ ] 길이·파라미터·중첩이 커졌다면 쪼갤 신호로 다뤘는가? (파라미터가 많으면 객체로 묶기)
...
- [ ] 처리할 수 있는 타입은 처리하고, 나머지는 컨텍스트를 보존해 다시 던지는가?
- [ ] 의도적 fail-open 에는 이유를 적은 주석이 있는가?
