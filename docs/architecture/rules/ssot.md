...
정본은 주입 룰 `plugins/common/rules/ssot.md` 다. 요지는 둘이다.

- 에러 타입·API 엔드포인트·환경변수는 한 곳에 정의하고 그 정의를 참조한다 — 복사한 값은
  한쪽만 고쳐져 어긋난다.
- 한 번의 변경에 여러 파일 수정이 필요하거나 같은 버그가 여러 곳에 나타나면 SSOT 위반 신호다.
...
에러 처리는 SSOT 원칙이 가장 자주 위반되는 영역입니다. 룰은 중앙 에러 핸들러를 **강제하지
않는다** — 프로젝트에 이미 있으면 그것을 거치라고만 한다. 모든 프로젝트에 중앙 핸들러를
신설하라는 요구는 작은 스크립트·라이브러리·훅처럼 그런 구조가 맞지 않는 코드에서 과잉이기
때문이다. 아래는 중앙 핸들러를 두기로 한 프로젝트에서 흔히 쓰는 구조의 **예시**다.
...
아래 필드는 구조화 로그를 설계할 때의 참고 예시이며, 룰이 요구하는 필드 목록이 아니다.
...
| (중앙 핸들러가 있다면) 이 에러가 그것을 통하는가? | catch 블록에 `console.log`만 있음   | 중앙 핸들러로 라우팅 |
| 이 값을 바꾸려면 몇 개 파일을 수정해야 하는가? | 한 번의 변경에 여러 파일 수정 필요  | SSOT 구조 설계 필요  |
